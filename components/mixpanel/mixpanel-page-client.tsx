...
export function MixpanelPageClient({
  ...
  async function connect() {
    if (!form.projectToken.trim()) {
      setMsg("Project Token is required");
      setMsgType("error");
      return;
    }
    setLoading(true);
      ...
      <Card>
        <CardHeader>
          <div className="flex items-center justify-between">
            <div>
              ...
              <CardDescription>Enter your Mixpanel project token to start tracking</CardDescription>
            </div>
        ...
        <CardContent className="space-y-4">
          ...
          <p className="text-xs text-gray-400">
            Find your Project Token in{" "}
            <span className="font-medium text-gray-500">Mixpanel → Settings → Project Settings</span>
          </p>
          ...
          {!connected && (
            ...
            >
              {showAdvanced ? "▾" : "▸"} Advanced — Service Account (optional, for reading funnels)
            </button>
          )}
      ...
      <Card>
        ...
        <CardContent>
          <ol className="space-y-3 text-sm text-gray-600">
            {[
              "Paste your Mixpanel Project Token above and click Connect",
...
mixpanel.init("${initialToken || 'YOUR_PROJECT_TOKEN'}", {track_pageview: true});
            ...
            <p className="text-xs text-gray-400 mt-2">
              Add this as a Custom HTML tag in GTM with an All Pages trigger.
            </p>
          </CardContent>
        </Card>
      )}
